...
export default function Connect({disableButton}){
    ...
    return <div className="w-full flex -mt-12">
        ...
        <div className="flex gap-4 w-full justify-center">
            {connectors.filter(c=>(c.name == "Coinbase Wallet" || c.name == "MetaMask")).map((connector) => {
                return<div  className={`${styles.window} !p-0 flex justify-center items-center cursor-pointer`} style={{transition:"all 1s"}} onClick={() => connect({ connector })} ><div className="p-2 w-max flex flex-col justify-center" key={connector.uid}>
